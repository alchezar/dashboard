...
use crate::model::types::{ApiServer, ApiUser, DbUser, IpConfig, NewUser};
use crate::prelude::{Error, Result, ServerStatus, ServiceStatus, VmRef};
...
RETURNING id"#,
        ...
        pub async fn test_template_id(transaction: &mut PgTransaction<'_>) -> Uuid {
            sqlx::query!(
                r#"
INSERT INTO templates (os_name, template_node, template_vmid, virtual_type)
VALUES ('os', 'node', 1, 'qemu')
RETURNING id"#
            )
            .fetch_one(transaction.as_mut())
            .await
